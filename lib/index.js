        ...
        async.each(resources, function(resource, callback){
            ...
            if(resource.hasOwnProperty('methods') && Array.isArray(resource.methods)){
                async.each(resource.methods, function(method, callback){
                    ...
                    request({
                        url: that.baseUri + util.preparers.prepareUri(resource.completeRelativeUri, method.uriParameters),
                        ...
                        responses[resource.relativeUri][method.method] = {err,response, parent: resource.parentUri};
                        return callback();
                    });
                }, function(err){
                    if(err){
